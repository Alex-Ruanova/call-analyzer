...
import { Icons, Sparkline } from "../components/components";
...
export default function DashboardScreen() {
  ...
  if (isLoading || !dashboard) {
    return <div style={{ padding: 28, color: "var(--text-3)" }}>Loading dashboard…</div>;
  }

  const sentimentValues = dashboard.sentiment_trend.map((s) => s.score);
  const callCountValues = dashboard.calls_per_day.map((d) => d.count);
    ...
    <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 22 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 12.5, color: "var(--text-3)" }}>
            An overview of your team's sales conversations.
          </p>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div className="dash-row" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14 }}>
        <div className="card">
          <div className="card__header">
            ...
            <div style={{ flex: 1 }} />
            <span className="pill" style={{ background: "rgba(16,185,129,0.1)", color: "var(--accent)", borderColor: "transparent" }}>
              +0.23
            </span>
          </div>
          <div className="card__body">
            <BigSparkline data={sentimentValues} height={170} />
          </div>
        </div>
        <div className="card">
          <div className="card__header">
          ...
          <div className="card__body">
            <BarChart data={callCountValues} height={170} />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card__header">
          ...
          <div style={{ flex: 1 }} />
          <button className="btn btn--ghost btn--sm" onClick={() => navigate("/calls")}>
...
function SentimentSpark({ sentiment }: { sentiment: number }) {
...
function BigSparkline({ data, height = 160 }: { data: number[]; height?: number }) {
  const w = 600;
  const pad = 24;
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height }}>
      {data.map((v, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, height: "100%" }}>
          <div style={{ flex: 1, width: "100%", display: "flex", alignItems: "flex-end" }}>
            <div
              style={{
                width: "100%",
                height: `${(v / max) * 100}%`,
                background: i === data.length - 1 ? "var(--accent)" : "rgba(16,185,129,0.4)",
          ...
          <span style={{ fontSize: 9, color: "var(--text-4)", fontVariantNumeric: "tabular-nums" }}>
            {data.length - i}
          </span>
        </div>
      ))}
